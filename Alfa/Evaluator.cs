    ...
    public class Evaluator
    {
        public Schedule EvaluateSchedule(Schedule schedule)
        {
            ...
            for (int subjectIndex = 0; subjectIndex < schedule.Scheduledays.Count; subjectIndex++)
            {
                ...
                if (day.Any(subject => subject.SubjectName != "" && day.Count(s => s.SubjectName == subject.SubjectName) > 1))
                ...
                if (profileSubjects.Any(s => s != day[0].SubjectName) || day.Skip(subjectsCount - 2).Any(subject => profileSubjects.Any(s => s != subject.SubjectName)))
                    totalPoints += 100;
                else totalPoints -= 100;
                ...
                if (day.Any(s => s.SubjectName == "TV" && day.Where(subject => subject.SubjectName != "").ToList()[subjectsCount - 1].SubjectName == "TV"))
                    totalPoints += 5000;
                else totalPoints -= 1000;
